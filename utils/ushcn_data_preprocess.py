"""
pre-processing script for the USHCN taken from
https://github.com/edebrouwer/gru_ode_bayes/blob/master/data_preproc/Climate/process.py
"""
import pandas as pd
...
def to_pandas(state_dir):
    ...
    values_list = list(chain.from_iterable(
        ("VALUE-" + str(i + 1), "MFLAG-" + str(i + 1), "QFLAG-" + str(i + 1), "SFLAG-" + str(i + 1)) for i in
        range(31)))
    columns += values_list
    ...
    df_p = df_n.pivot_table(values='value', index=["COOP_ID", "YEAR", "MONTH", "DAY", "ELEMENT"], columns="TYPE",
                            aggfunc="first")
...
def clean_and_subsample():
    ...
    # Create a time_index.
    import datetime
    sample_df["DATE"] = pd.to_datetime((sample_df.YEAR * 10000 + sample_df.MONTH * 100 + sample_df.DAY).apply(str),
                                       format='%Y%m%d')
